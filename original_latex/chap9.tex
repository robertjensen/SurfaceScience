...
\lambda(Å) &=& \sqrt{\frac{150.4}{E_p(eV)}}
\end{eqnarray}
...
\end{equation}
where $f(\alpha,\phi)$ is the amplitude of the outgoing electron and $\mid f(\alpha,\phi) \mid^2$ is the differential scattering cross section which must be found by solving the Schrødinger equation in the vicinity of the atom where V $\neq$ 0. $\alpha $ and $\phi$ are the angles of the scattering. Assuming that the potential is spherical symmetrical the solutions to the Schrødinger equations are separable in the coordinates r,$\alpha, \phi$ and the wave functions are of the hydrogen type namely a product of radial and spherical harmonic:
\begin{equation}
